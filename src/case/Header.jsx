...
export default function Header () {
  ...
  return (
    <header className='header'>
      ...
      <button type="button" className={`btn ${mode ? "btn-success" : "btn-secondary"}`} onClick={mode ? () => alert("Вы уже зарегестрировались!") : () => authorizationUser(authorizationAddress)}>{mode ? "Вы авторизированы" : "Аторизация пользователя"}</button>
    </header>
  )
}